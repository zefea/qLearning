...
class Train:
    ...
    def __init__(self,rewards,env_row,env_column,decision,initialQ,id):
      
        self.id = id
        self.rewards = rewards
        self.env_row = env_row
        self.env_column = env_column
        self.initQtable(env_row,env_column,initialQ)
    ...
    def initQtable(self,row,column,initialQ):
        states = row * column
        self.q_table  = np.full((states,4),initialQ)
    ...
    def takeActionNormal(self,act,curr_row, curr_col):
    ...
    def takeAction(self,act,curr_row, curr_col):
        new_curr_row = curr_row
        new_curr_col = curr_col
        if self.actions[act] == 'up':
            if curr_row > 0:
                new_curr_row -= 1
            elif curr_row == 0:
                new_curr_row = self.env_row-1
        elif self.actions[act] == 'right':
            
            if curr_col < self.env_column-1:
                new_curr_col += 1 
            elif curr_col == self.env_column-1:
                new_curr_col = 0

        elif self.actions[act] == 'down':
             
            if curr_row < self.env_row-1:
                new_curr_row += 1
            elif curr_row == self.env_row-1:
                new_curr_row = 0

        elif self.actions[act] == 'left':
            
            if curr_col > 0:
                new_curr_col -= 1  
            elif curr_row == 0:
                new_curr_col = self.env_column-1
